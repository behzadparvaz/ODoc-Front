    ...
    >
      ...
      <ul className="w-full items">
        {navigaitionItemsVal?.map((item: any) => {
          ...
          return (
            <div key={item?.id}>
              ...
              >
                <NextLink href={item?.link} className={`${isActive ? 'cursor-default' : ''} py-2 gap-x-1 relative flex transition-all duration-200`}>
                  {item?.icon}
                  {item?.text}
                </NextLink>
              </li>
        ...
        <li className={` mb-3 py-2 block mx-auto text-md relative`}>
          <Link href={routeList?.logoutRoute} className={`text-red-600 z-10 py-2 flex items-center gap-x-2 relative transition-all duration-200`}>
