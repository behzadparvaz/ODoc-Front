...
const OrderHistory = () => {
  ...
  const onChangeFilter = (event) => {
    push({ query: { ...query, statusId: event.target.value } }, undefined, { shallow: true });
  };

  const transformDataToOptions = useMemo(
    () => orderStatuses?.map(item => ({ name: statusesDictionary[item.name], id: item.id })) ?? []
    , [orderStatuses]);
  ...
  return (
    <MainLayout headerChildren={headerChildrenElement} title="تاریخچه سفارش ها">
      <div className="my-10 px-6 flex justify-end items-center gap-3">
        <span>فیلتر:</span>
        <Select name={'statusId'} value={statusId} options={transformDataToOptions} onChange={onChangeFilter} className={'w-full sm:w-1/3'}/>
      </div>
      {isLoading === false ? <div className="w-full px-6 pb-8 relative">
          {orderHistoryData?.map((item) => {
            return (
              <OrderItem key={item?.id}
                         handleClikOnPaymentButton={() => handleClickOnPaymentButton(item?.orderCode, item?.finalPrice)}
                         data={item}/>
            );
          })}
        </div>
