...
const Addresses = () => {
  ...
  return (
    <ProfileLayout hasBackBtn title={profileText?.addresses}>
      {isLoading === false ? <>
          {!addressItem?.length ?
            <div className="text-red text-sm text-red-600 text-center py-8">در حال حاضر آدرسی برای شما ثبت نشده
              است</div> : null}
          <Button handleClick={() => handleClickOpenModal()} className={`${addressItem?.length ? 'mb-4' : 'w-full'} mt-4`}
                  size="large" buttonType="contained" variant={'primary'}>افزودن آدرس</Button>
