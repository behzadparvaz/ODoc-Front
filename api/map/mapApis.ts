...
export const parsiMapReverseLocation = async (body: IReverseLocationBody<string>) => {
  delete axios.defaults.headers.common['Authorization'];
  const data = await axios.get(
    `${PARSI_API_URL}geocode/reverse?key=${parsiMapAccessToken}&location=${body.location}&plate=true`
  );
  return await data?.data;
};
