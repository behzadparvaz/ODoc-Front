...
const TenderShipmentDetail = ({
  ...
  const closingTime = vendor?.toTimeActive
    ...
    ?.join?.(':');
  return (
    <div className="w-full h-[120px] bg-white p-4 py-2 rounded-base flex flex-col gap-y-3 shadow-md">
      <div className="flex items-center justify-between">
        <span className="text-sm font-medium leading-7">
          {vendor.isShowName ? vendor?.vendorName : vendor.secondaryName}
        </span>
        <span className="w-16 h-6 rounded-full flex justify-center items-center bg-orange-50 text-xs font-normal text-orange-400 leading-6 gap-x-2">
          {/* {'vendor?.rate'} */}
          {4}
